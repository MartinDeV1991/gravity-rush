...
class Tilemap:
    ...
    def find_highest_block_position(self):
        highest_y = float("inf")
        highest_block_position = None

        for tile in self.tilemap:
            tile_y = self.tilemap[tile]["pos"][1]
            if tile_y < highest_y:
                highest_y = tile_y
                highest_block_position = self.tilemap[tile]["pos"]
        return highest_block_position

    def find_leftest_block_position(self):
        leftest_x = float("inf")
        leftest_block_position = None

        for tile in self.tilemap:
            tile_x = self.tilemap[tile]["pos"][0]
            if tile_x < leftest_x:
                leftest_x = tile_x
                leftest_block_position = self.tilemap[tile]["pos"]
        return leftest_block_position

    def find_rightest_block_position(self):
        rightest_x = float("-inf")
        rightest_block_position = None

        for tile in self.tilemap:
            tile_x = self.tilemap[tile]["pos"][0]
            if tile_x > rightest_x:
                rightest_x = tile_x
                rightest_block_position = self.tilemap[tile]["pos"]
        return rightest_block_position
